...
public:
  BilateralTestNode(ros::NodeHandle& nh)
    ...
    nh_.param("Kc", Kc_, 30.0);
    nh_.param("Kd", Kd_, 5.0);
    nh_.param("Kl", Kl_, 10.0);
    ...
    timer_ = nh_.createTimer(ros::Duration(0.1), &BilateralTestNode::controlLoop, this);
  }
...
private:
  ...
  void controlLoop(const ros::TimerEvent&) {
    ...
    if(!initialized_position_second_){
      ROS_WARN("Position not initialized for second stage. Skipping control loop.");
      return;
    }
